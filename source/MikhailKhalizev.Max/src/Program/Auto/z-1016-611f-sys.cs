using System;
using MikhailKhalizev.Processor.x86.BinToCSharp;

namespace MikhailKhalizev.Max.Program
{
    public partial class RawProgram
    {
        [MethodInfo("ad8a21fc-6b61-4a37-9fb7-e2bc8eea7bea")]
        public void /* sys */ Method_1016_611f()
        {
            ii(0x1016_611f, 1); pushd(edx);                             /* push edx */
            ii(0x1016_6120, 6); mov(edx, memd_a32[ds, 0x101b_e7bc]);    /* mov edx, [0x101be7bc] */
            ii(0x1016_6126, 2); mov(memd_a32[ds, eax], edx);            /* mov [eax], edx */
            ii(0x1016_6128, 5); mov(memd_a32[ds, 0x101b_e7bc], eax);    /* mov [0x101be7bc], eax */
            ii(0x1016_612d, 1); popd(edx);                              /* pop edx */
            ii(0x1016_612e, 1); retd(); return;                         /* ret */
        //  ii(0x1016_612f, 1); Недостижимый (и не декодированный) код.
        //    ii(0x1016_6130, 1); pushd(ebx);                             /* push ebx */
        //    ii(0x1016_6131, 1); pushd(ecx);                             /* push ecx */
        //    ii(0x1016_6132, 2); mov(ebx, eax);                          /* mov ebx, eax */
        l_0x1016_6134:
            ii(0x1016_6134, 2); mov(al, memb_a32[ds, ebx]);             /* mov al, [ebx] */
            ii(0x1016_6136, 2); xor(ecx, ecx);                          /* xor ecx, ecx */
            ii(0x1016_6138, 2); mov(ah, memb_a32[ds, edx]);             /* mov ah, [edx] */
            ii(0x1016_613a, 2); mov(cl, al);                            /* mov cl, al */
            ii(0x1016_613c, 3); cmp(ecx, 0x41);                         /* cmp ecx, 0x41 */
            ii(0x1016_613f, 2); if(jld(0x1016_6148, 0x7)) goto l_0x1016_6148; /* jl 0x10166148 */
            ii(0x1016_6141, 3); cmp(ecx, 0x5a);                         /* cmp ecx, 0x5a */
            ii(0x1016_6144, 2); if(jgd(0x1016_6148, 0x2)) goto l_0x1016_6148; /* jg 0x10166148 */
            ii(0x1016_6146, 2); add(al, 0x20);                          /* add al, 0x20 */
        l_0x1016_6148:
            ii(0x1016_6148, 2); xor(ecx, ecx);                          /* xor ecx, ecx */
            ii(0x1016_614a, 2); mov(cl, ah);                            /* mov cl, ah */
            ii(0x1016_614c, 3); cmp(ecx, 0x41);                         /* cmp ecx, 0x41 */
            ii(0x1016_614f, 2); if(jld(0x1016_6159, 0x8)) goto l_0x1016_6159; /* jl 0x10166159 */
            ii(0x1016_6151, 3); cmp(ecx, 0x5a);                         /* cmp ecx, 0x5a */
            ii(0x1016_6154, 2); if(jgd(0x1016_6159, 0x3)) goto l_0x1016_6159; /* jg 0x10166159 */
            ii(0x1016_6156, 3); add(ah, 0x20);                          /* add ah, 0x20 */
        l_0x1016_6159:
            ii(0x1016_6159, 2); cmp(al, ah);                            /* cmp al, ah */
            ii(0x1016_615b, 2); if(jnzd(0x1016_6165, 0x8)) goto l_0x1016_6165; /* jnz 0x10166165 */
            ii(0x1016_615d, 2); test(ah, ah);                           /* test ah, ah */
            ii(0x1016_615f, 2); if(jzd(0x1016_6165, 0x4)) goto l_0x1016_6165; /* jz 0x10166165 */
            ii(0x1016_6161, 1); inc(ebx);                               /* inc ebx */
            ii(0x1016_6162, 1); inc(edx);                               /* inc edx */
            ii(0x1016_6163, 2); jmpd(0x1016_6134, -0x31); goto l_0x1016_6134; /* jmp 0x10166134 */
        l_0x1016_6165:
            ii(0x1016_6165, 2); xor(edx, edx);                          /* xor edx, edx */
            ii(0x1016_6167, 2); mov(dl, al);                            /* mov dl, al */
            ii(0x1016_6169, 2); mov(al, ah);                            /* mov al, ah */
            ii(0x1016_616b, 5); and(eax, 0xff);                         /* and eax, 0xff */
            ii(0x1016_6170, 2); sub(edx, eax);                          /* sub edx, eax */
            ii(0x1016_6172, 2); mov(eax, edx);                          /* mov eax, edx */
            ii(0x1016_6174, 1); popd(ecx);                              /* pop ecx */
            ii(0x1016_6175, 1); popd(ebx);                              /* pop ebx */
            ii(0x1016_6176, 1); retd(); return;                         /* ret */
        }
    }
}
