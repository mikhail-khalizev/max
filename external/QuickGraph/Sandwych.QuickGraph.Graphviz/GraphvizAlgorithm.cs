...
*/
        ...
        public Func<TVertex, bool> Filter { get; set; } = v => true;
        ...
        public string Generate()
        {
            ...
            // build vertex id map
            int i = 0;
            foreach (TVertex v in this.VisitedGraph.Vertices.Where(x => Filter(x)))
            ...
            foreach (var v in VisitedGraph.Vertices.Where(x => Filter(x)))
                colors[v] = GraphColor.White;
            var edgeColors = new Dictionary<TEdge, GraphColor>();
            foreach (var e in VisitedGraph.Edges.Where(x => Filter(x.Source) && Filter(x.Target)))
                edgeColors[e] = GraphColor.White;

            WriteVertices(colors, VisitedGraph.Vertices.Where(x => Filter(x)));
            WriteEdges(edgeColors, VisitedGraph.Edges.Where(x => Filter(x.Source) && Filter(x.Target)));
